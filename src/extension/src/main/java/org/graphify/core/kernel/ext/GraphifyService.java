package org.graphify.core.kernel.ext;
...
public class GraphifyService {
    ...
    @POST
      ...
      public Response extractFeatures(String body, @Context GraphDatabaseService db) throws IOException {
        ...
        if (input.containsKey("focus")) {
            labeledText.setFocus((Integer) input.get("focus"));
        } else {
    ...
    @Path("/targets/create")
    @Produces(MediaType.APPLICATION_JSON)
    public Response createTarget(String body, @Context GraphDatabaseService db) throws IOException {
        SelectedFeatures selectedFeatures;

        // Deserialize the request body
        try {
            selectedFeatures = objectMapper.readValue(body, SelectedFeatures.class);
        } catch (Exception e) {
            return Response.status(500).entity("{\"error\":\"" + Arrays.toString(e.getStackTrace()) + "\"}").build();
        }

        FeatureTargetResponse featureTargetResponse;

        try {
            // Create a feature target
            featureTargetResponse = FeatureSelector.createFeatureTarget(selectedFeatures, db);
        ...
        Gson gson = new Gson();
        String response = gson.toJson(featureTargetResponse);
